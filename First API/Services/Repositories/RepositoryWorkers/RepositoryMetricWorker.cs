...
namespace MetricsMeneger.Services.Repositories
{
    public class RepositoryMetricWorker
    {
        ...
        public void Create(Metric item)
        {
            ...
            connection.Execute($"CREATE TABLE IF NOT EXISTS {item.CategoryName} (id INTEGER PRIMARY KEY AUTOINCREMENT, CategoryName TEXT, InstanceName TEXT,CounterName TEXT, Value INTEGER, Time INTEGER)");
            connection.Execute($"INSERT INTO {item.CategoryName} (CategoryName, InstanceName, CounterName, Value, Time) VALUES(@CategoryName, @InstanceName, @CounterName, @Value, @Time)",
